...
SchemaRegistry.prototype._processSelectedTopics = Promise.method(function () {
  ...
  return this._checkForRegex(rexp)
    .then((subjectList) => {
      this.selectedTopics.forEach(function (selectedTopic) {
        if (rexp.test(selectedTopic)) {
          var topicRegex = new RegExp(selectedTopic);
 ...
 * @private
 */
SchemaRegistry.prototype._checkForRegex = Promise.method(function (rexp) {
  if (this.selectedTopics.some((t) => { return rexp.test(t); })) {
    return this._fetchAllSchemaTopics();
  }
  return [];
});
