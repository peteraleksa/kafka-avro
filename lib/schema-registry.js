...
/**
 * Function for creating unique set on array
 *
 * @param value
 * @param index
 * @param self
 * @return {boolean} unique array
 * @private
 */
SchemaRegistry.prototype._unique = function (value, index, self) {
 ...
 * @private
 */
SchemaRegistry.prototype._isRegexTopic = function (topic) {
  return /[$*^]/.test(topic);
};

/**
 * Check selected topics for regex and fetch all topics if regex found.
 *
 * @return {Array.<string>} An array of properly formatted strings to be used as
 *    topics when matching the regex.
 * @private
 */
SchemaRegistry.prototype._checkForRegex = Promise.method(function () {
  if (this.selectedTopics.some(this._isRegexTopic)) {
    return this._fetchAllSchemaTopics();
  }
  return [];
});
 ...
 * @private
 */
SchemaRegistry.prototype._processRegexTopic = function (regexTopic, subjectList) {
  var topics = [];
  var topicRegex = new RegExp(regexTopic);
  subjectList.forEach(function (subject) {
    if (/-value$/.test(subject) && topicRegex.test(subject.replace('-value', ''))) {
      topics.push(subject);
    }
    if (/-key$/.test(subject) && topicRegex.test(subject.replace('-key', ''))) {
      topics.push(subject);
    }
  });
  return topics;
 ...
 *
 * @return {Promise(Array.<string>)} An array of properly formated strings to be used as
...
SchemaRegistry.prototype._processSelectedTopics = Promise.method(function () {
  var topics = [];
  var self = this;
  return this._checkForRegex()
    .then((subjectList) => {
      this.selectedTopics.forEach(function (selectedTopic) {
        if (self._isRegexTopic(selectedTopic)) {
          topics = topics.concat(self._processRegexTopic(selectedTopic, subjectList));
        } else {
          topics.push(selectedTopic + '-value');
          topics.push(selectedTopic + '-key');
        }
      });
      return topics.filter(this._unique);
    });
});
...
/**
 * Fetch a single schema by id from the SR and return its value along with metadata.
 *
 * @param {number} schemaId The id to fetch from the SR.
 * @return {Promise(Object)} A Promise with an object containing the
 *    schema.
 * @private
 */
SchemaRegistry.prototype._fetchSchemaById = Promise.method(function (schemaId) {
  var fetchSchemaUrl = new URL(
    'schemas/ids/' + schemaId,
    this.schemaRegistryUrl
  ).toString();

  log.debug('_fetchSchemaById() :: Fetching schema url:',
    fetchSchemaUrl);

  return Promise.resolve(instance.get(fetchSchemaUrl))
    .then((response) => {
      log.debug('_fetchSchemaById() :: Fetched schema url:',
        fetchSchemaUrl);
